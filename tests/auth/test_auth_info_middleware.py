...
@pytest.mark.asyncio
async def test_on_call_tool_rejects_user_not_on_allowlist(monkeypatch):
    monkeypatch.setenv(
    ...
    async def call_next(ctx):
        raise AssertionError("call_next should not run for blocked users")

    with pytest.raises(AuthorizationError, match="chirag@infrasingularity.com"):
